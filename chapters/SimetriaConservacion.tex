...
\subsection{Ejemplo}
Tenemos la acción
\[S = \int_{t_A}^{t_B} -mc^2 \sqrt{1-\frac{\dot{x}^2}{c^2}}dt\]
...
\[H = \frac{\partial \pazocal{L}}{\partial \dot{x}} \dot{x} - \pazocal{L} = \gamma m \dot{x}^2+\frac{mc^2}{\gamma} = \gamma m \left(\dot{x}^2+c^2\left(1-\frac{\dot{x}^2}{c^2}\right)\right) = +\gamma m c^2 \ \ \ \ \ \ \frac{\partial \pazocal{L}}{\partial \dot{x}} = \gamma m \dot{x}\]
Por lo tanto, la cantidad conservada será
\[I =\gamma m c x- \gamma m \dot{x} ct = H \frac{x}{c} - pct\]