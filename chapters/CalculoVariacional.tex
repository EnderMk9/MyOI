...
Tenemos una función $f:\mathbfcal{U}\in \mathbb{R} \mapsto f(x)\in \mathbb{R}$, donde tanto el dominio $\mathbfcal{U}$ como la imagen pertencen a $\mathbb{R}$.
En contraposición, un funcional es una función $F: \mathbfcal{f} \in \mathcal{F}\{x,\mathbb{R}\} \mapsto F[f]\in \mathbb{R}$, donde $\mathcal{F}\{x,\mathbb{R}\}$ es el conjunto de todas las funciones reales de una variable \sidenote[]{Aunque se puede definir un funcional como una función de $\mathcal{F}\{x,\mathbb{R}\}^n$ para $n$ funciones reales.}, tal que la imagen es un número real.
...
\begin{marginfigure}[0cm]
	...
	\labfig{margin1}
\end{marginfigure}
Definimos $\delta y(x)\equiv\bar{y}(x)-y(x) \inlineeqnum \label{1.1.1} $, donde $\bar{y}$ es el camino variado e $y$ es el camino de referencia. Supondremos que el camino de referencia es el camino que extremiza el funcional, entonces una pequeña variación $\delta y$ no debería alterar el funcional.

\hypertarget{1.1.2}{\phantom{}}
Podemos parametrizar $\delta y(x) \equiv a \eta(x) \label{1.1.2}\inlineeqnum$\hypertarget{1.1.3}{\phantom{}}, donde $a$ es un parámetro independiente de $x$ y $\eta(x)=\delta y(x)/a$ es una función arbitraria que da forma el camino variado y que debe cumplir que $\eta(x_A)=\eta(x_B)=0 \label{1.1.3} \inlineeqnum$ para verificar las condiciones que hemos impuesto en \eqref{1.0.2}, ya que todo camino, sea el de referencia o el variado, debe cumplirlas.

Definimos entonces una nueva función $Y(x,a)\equiv y(x)+a\eta(x) \label{1.1.4} \inlineeqnum$ tal que $Y(x,0)=y(x)$ y $Y(x,a)=\bar{y}(x)$. Si derivamos esta función con repecto a $a$, y con respecto a $x$ tenemos
\begin{equation}\label{1.1.5}
\frac{\partial Y}{\partial a}=\eta(x) ; \ \ \frac{\partial Y}{\partial x}=y'(x)+a\eta'(x)\equiv Y'(x,a); \ \  \frac{\partial Y'}{\partial a} = \eta ' (x)
\end{equation} \refstepcounter{subsection}
Podemos definir ahora $\delta y'(x) \equiv \bar{y}'(x)-y'=Y'(x,a)-Y'(x,0)$, que por la expresión anterior nos resulta $\delta y'(x)=a \eta'(x) \label{1.1.6} \inlineeqnum$.
Combinando ahora \eqref{1.1.6} y \hyperlink{1.1.2}{(1.1.2)} podemos llegar a la conclusión de que la derivada y $\delta$ conmutan
...
\end{equation} \refstepcounter{subsection}
Donde $\delta \mathbb{g}$ es la variación primera de la función, que podemos reescribir desarrollando la derivada usando la regla de la cadena, y usamos \hyperlink{1.1.2}{(1.1.2)} y \eqref{1.1.6}
...
De \eqref{1.1.12} sustuimos en \eqref{1.1.14}, sacamos factor común el parámetro $a$ e integramos por partes el segundo término, tal que $ u = \partial_{y'}g$ y $ dv = \eta' dx$
...
\end{equation} \refstepcounter{subsection} 
Por \hyperlink{1.1.3}{(1.1.3)} el segundo término es 0, juntando las integrales y usando \hyperlink{1.1.2}{(1.1.2)}
...
\vspace{-20pt}
\Large\begin{equation} \label{EL}
    \boxed{\frac{\partial g}{\partial y} -\frac{d}{dx}\left(\frac{\partial g}{\partial y'}\right) =0} \iff \delta F =0 \tag{1.2.3/E-L}
\end{equation} \refstepcounter{subsection}\normalsize
...
\end{equation} \refstepcounter{subsection}
Y como la variación en los extremos se anula \hyperlink{1.1.3}{(1.1.3)}, se verifica que $\delta \tilde{F} = \delta F$, y por lo tanto \eqref{EL} permanece invariante bajo esta transformación.
%-------------------------------------------------------------------------------------------
\subsubsection{Geodésica del plano}
Un ejemplo para aplicar \eqref{EL} es minimizar la distancia $d=\int{ds}$ en el plano ecuclídeo. Si $y=y(x)$, entonces $ds=\sqrt{dx^2+dy^2}=\sqrt{1+y'^2}dx=g dx$, tal que
...
Podemos reescribir \eqref{EL} de otra forma que nos va resultar últil para resolver algunos problemas y va a resultar muy importante en episodios posteriores al definir el \textit{Hamiltoniano}
...
Podemos observar que el término en el primer miembro de la segunda expresión aparece en \eqref{EL} sin multiplicar por $y'$.
...
\section{Generalización a varias variables} \label{sec:1.3} \refstepcounter{subsection} 
Denotamos $\{f_\alpha(x)\}$ a un conjunto de $N$ funciones distintas, que verifican una expresión similar a \eqref{1.0.2}, $f_\alpha(x_A)=f_{\alpha A}; \ f_\alpha(x_B)=f_{\alpha B} \label{1.3.1} \inlineeqnum$.
...
Ahora siguiendo un desarrollo idéntico a \eqref{1.1.12}, desarrollando la regla de la cadena para cada una de las variables de $g$ resulta en un sumatorio y los argumentos siguientes para llegar a $\delta g$ son idénticos puesto que son lineales, de tal forma llegamos a la siguiente expresión
\begin{equation} \label{1.3.2}
    \delta g = \sum{\frac{\partial g}{\partial f_\alpha} \delta f_\alpha + \frac{\partial g}{\partial f'_\alpha} \delta f'_\alpha}
\end{equation} \refstepcounter{subsection} 
La expresión \eqref{1.1.14} no dependía de las variables de $g$, por lo que es directamente aplicable, sustituyendo \eqref{1.3.2} y haciendo la regla de la cadena igual que en \eqref{1.2.1} llegamos a una expresión similar a \eqref{1.2.2}, usando que la integral conmuta con el sumatorio
...
\vspace{-20pt}
\Large\begin{equation} \label{1.3.4}
    \boxed{\frac{\partial g}{\partial f_\alpha} -\frac{d}{dx}\left(\frac{\partial g}{\partial f'_\alpha}\right) =0} \iff \delta F =0 \tag{1.3.4/E-L}
\end{equation} \refstepcounter{subsection}\normalsize
...
\end{equation} \refstepcounter{subsection}
De esta forma, hemos encontrado la dependencia explícita de $\delta f_\beta$ en función de los $\delta f_\gamma$, estos últimos siendo independientes entre sí. Ahora tomamos \eqref{1.3.3} y renombramos el factor en corchetes por $\Gamma_\alpha$ y separamos como en \eqref{1.3.6}
...
\end{equation} \refstepcounter{subsection}
Sustituyendo $\delta f_\beta$ de \eqref{1.3.7}
...
\end{equation} \refstepcounter{subsection} 
Definimos entonces $\lambda_a=\sum_{\beta=N-m+1}^{N}{\Gamma_\beta J^{-1}_{\beta a}}$ como los multiplicadores de \textit{Lagrange} y reemplazando $a_{a\gamma}$ por su definición de \eqref{1.3.5}
...
\[\Gamma_\gamma -{\sum_{a=1}^m{\lambda_a \frac{\partial G_a}{\partial f_\gamma}}}=\Gamma_\gamma -{\sum_{a=1}^m{\lambda_a J_{a\gamma}}} = \Gamma_\gamma -{\sum_{a=1}^m{\sum_{\beta=N-m+1}^{N}{\Gamma_\beta J^{-1}_{\beta a}} J_{a\gamma}}}=\]
\begin{equation} \label{1.3.14}
    =\Gamma_\gamma -\sum_{\beta=N-m+1}^{N}{\Gamma_\beta \delta_{\beta \gamma}} = \Gamma_\gamma - \Gamma_\gamma = 0
\end{equation} \refstepcounter{subsection}
También se verifica \eqref{1.3.12}, por lo que entonces
\Large\begin{equation} \label{LG}
    \boxed{\frac{\partial g}{\partial f_\alpha} -\frac{d}{dx}\left(\frac{\partial g}{\partial f'_\alpha}\right) ={\sum_{i=1}^m{\lambda_i \frac{\partial G_i}{\partial f_\alpha}}} \ \ \ \ \ G_i(\{f_\alpha\}) = 0} \tag{1.3.15/L-M}
\end{equation} \refstepcounter{subsection}\normalsize
